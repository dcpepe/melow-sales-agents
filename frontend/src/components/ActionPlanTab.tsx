...
export default function ActionPlanTab({ analysisId, dealId }: { analysisId?: string; dealId?: string }) {
  ...
  // Cache-first: try to load cached action plan
  useEffect(() => {
    if (!dealId) { setInitialLoad(false); return; }
    fetch("/api/jobs", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ deal_id: dealId, type: "action_plan", force: false }),
    })
      .then((r) => r.json())
      .then((data) => {
        if (data.cached && data.version?.content) {
          try {
            setPlan(JSON.parse(data.version.content));
    ...
    try {
      if (dealId) {
        const res = await fetch("/api/jobs", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ deal_id: dealId, type: "action_plan", force: true }),
        });
        ...
        if (data.version?.content) {
          ...
          setError(data.job.error || "Failed");
        }
      } else {
  ...
  if (initialLoad) return <div className="bg-white rounded-xl border p-8 text-center"><p className="text-sm text-gray-400">Loading...</p></div>;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-gray-900">Sales Demon Mode</h3>
